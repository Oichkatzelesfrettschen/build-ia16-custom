...
rm -rf redist-djgpp/appinfo redist-djgpp/devel redist-djgpp/source
...
FIN
  ...
  # linker scripts yet --- we cannot have two separate files elf_i386.xdc
  # and elf_i386.xdce in the same ldscripts/ directory under MS-DOS's 8.3
  # filename restrictions.
  #
  # Currently I exclude (most of) the `d' and `s' scripts, since `d'
  # corresponds to the `-pie' linker option, and `-s' to the `-shared'
  # option, neither of which make much sense (yet) in the IA-16 context.
  #
  # Also, the elf_i386_msdos_mz.x* scripts are the same as the elf_i386.x*
  # ones.
  rm -f -v redist-djgpp/i16butil.zip \
    redist-djgpp/devel/i16gnu/ia16-elf/lib/ldscripts/elf_i386.x[ds][^e]* \
  ...
  rm -f redist-djgpp/i16budoc.zip
  (cd redist-djgpp && \
...
if in_list newlib BUILDLIST; then
  ...
  rm -f redist-djgpp/i16newli.zip
...
if in_list libi86 BUILDLIST; then
  ...
  rm -f redist-djgpp/i16lbi86.zip
...
FIN
  ...
  rm -f redist-djgpp/i16gcdoc.zip
  (cd redist-djgpp && \
    find -L . \( -name '*.1' -o -name '*.info' -o -name '*.lsm' \) -print0 | \
  ...
  if (cd redist-djgpp && zip -d i16gcc.zip '*LTO*' &>/dev/null); then
    ...
    rm -f redist-djgpp/i16gclto.zip
    (cd redist-djgpp && \
